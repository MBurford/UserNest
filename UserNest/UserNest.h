/*
 ...
 After creating, present normally:
    UserNest	*userNest = [[UserNest alloc] initWithAppID:@"YOUR_APP_ID" delegate:self];
 ...
 More advanced but even better, checking if they are already logged in:
    UserNest	*userNest = [[UserNest alloc] initWithAppID:@"674062da" completionHandler:^(Boolean loggedIn, NSDictionary *loginData) {
        if (loggedIn) {
...
@protocol UserNestDelegate <NSObject>
@optional
...
@interface UserNest : UIViewController
 ...
 */
@property (nonatomic, assign) id<UserNestDelegate> delegate;
